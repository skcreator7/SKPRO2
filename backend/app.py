# ============================================================================
# 🚀 SK4FiLM v8.2 - SUPABASE THUMBNAIL STORAGE INTEGRATED
# ============================================================================
...
class SupabaseStorage:
    ...
    def __init__(self):
        ...
        self.supabase_bucket = os.environ.get("SUPABASE_BUCKET", "sk4film-thumbnails")
        self.enabled = bool(self.supabase_url and self.supabase_key)
        
        if self.enabled:
            logger.info("✅ Supabase Storage: ENABLED")
            logger.info(f"   Bucket: {self.supabase_bucket}")
        else:
            logger.warning("⚠️ Supabase Storage: DISABLED (Set SUPABASE_URL and SUPABASE_KEY)")
    
    async def upload_thumbnail(self, image_bytes: bytes, file_id: str, message_id: int, title: str = "") -> Optional[str]:
        ...
        try:
            ...
            safe_title = re.sub(r'[^\w\-]', '_', title[:50]) if title else "video"
            filename = f"{safe_title}_{file_id}_{unique_id}.jpg"
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(upload_url, headers=headers, data=image_bytes) as response:
                    if response.status in [200, 201]:
                        # Return public URL
                        public_url = f"{self.supabase_url}/storage/v1/object/public/{self.supabase_bucket}/{filename}"
...
# ============================================================================
# ✅ VIDEO THUMBNAIL EXTRACTOR WITH SUPABASE
# ============================================================================

class VideoThumbnailExtractor:
    """Telegram video files से thumbnail extract और Supabase में store करे"""
    
    @staticmethod
    async def extract_and_store_thumbnail(message, file_info: dict) -> dict:
        """
        Telegram से thumbnail extract करे और Supabase में store करे
        Returns: thumbnail info dictionary
        """
        try:
            ...
            # 1. Telegram से thumbnail extract करें
            if message.video:
                # Video message के लिए
                if hasattr(message.video, 'thumbnail') and message.video.thumbnail:
                    try:
                        # Thumbnail file download करें
                        thumb_file = await Bot.download_media(
                            message.video.thumbnail,
                            in_memory=True
                        )
                        if thumb_file:
                            thumbnail_data = thumb_file.getvalue()
                            thumbnail_source = "telegram_video_thumb"
                            logger.debug(f"✅ Telegram video thumbnail extracted: {message.id}")
                    except Exception as e:
                        logger.debug(f"Video thumbnail download failed: {e}")
                
                # Fallback: Video के first frame से thumbnail
                if not thumbnail_data:
                    try:
                        # Video का छोटा preview download करें
                        video_preview = await Bot.download_media(
                            message,
                            in_memory=True,
                            file_name="preview.jpg"
                        )
                        if video_preview:
                            thumbnail_data = video_preview.getvalue()
                            thumbnail_source = "video_first_frame"
                            logger.debug(f"✅ Video first frame extracted: {message.id}")
                    except Exception as e:
                        logger.debug(f"Video frame extraction failed: {e}")
            
            elif message.document:
                # Document video के लिए
                ...
                if 'video' in mime_type.lower():
                    if hasattr(message.document, 'thumbnail') and message.document.thumbnail:
                        try:
                            ...
                            if thumb_file:
                                thumbnail_data = thumb_file.getvalue()
                                thumbnail_source = "telegram_document_thumb"
                                logger.debug(f"✅ Document thumbnail extracted: {message.id}")
                        except Exception as e:
                            logger.debug(f"Document thumbnail failed: {e}")
            
            # 2. Agar thumbnail extract हुआ, तो Supabase में upload करें
            if thumbnail_data and supabase_storage.enabled:
                try:
                    # Upload to Supabase
                    ...
                    supabase_url = await supabase_storage.upload_thumbnail(
                    ...
                    if supabase_url:
                        # Success - Supabase URL return करें
                        return {
                            'has_thumbnail': True,
                    ...
                    # Fallback: Base64 में store करें
                    return {
                        'has_thumbnail': True,
            ...
            # 3. Agar Supabase नहीं है, तो Base64 में store करें
            elif thumbnail_data:
                return {
                    'has_thumbnail': True,
                    'thumbnail_data': base64.b64encode(thumbnail_data).decode('utf-8'),
            ...
            # 4. No thumbnail found
            return {
                'has_thumbnail': False,
                'thumbnail_source': 'not_found',
                'extracted_at': datetime.now().isoformat()
            }
    ...
    @staticmethod
    async def get_thumbnail_for_file(file_info: dict) -> Optional[str]:
        """
        File info से thumbnail URL प्राप्त करें
        """
        if not file_info.get('has_thumbnail'):
            return None
        
        thumbnail_info = file_info.get('thumbnail_info', {})
        
        # Supabase URL
        if thumbnail_info.get('stored_in') == 'supabase':
            return thumbnail_info.get('thumbnail_url')
        
        # Base64 data
        elif thumbnail_info.get('stored_in') == 'base64':
            # Create data URL
            thumbnail_data = thumbnail_info.get('thumbnail_data', '')
            if thumbnail_data:
...
async def add_headers(response):
    ...
    response.headers['X-SK4FiLM-Version'] = '8.2-SUPABASE-THUMBNAILS'
...
async def index_single_file_smart(message):
    ...
    try:
        ...
        # For video files: Extract and store thumbnail in Supabase
        thumbnail_info = {}
        if file_info.get('is_video_file', False):
            logger.info(f"🎬 Video file detected: {file_info.get('title', 'Unknown')}")
            
            # Extract and store thumbnail
            thumbnail_info = await VideoThumbnailExtractor.extract_and_store_thumbnail(
                message, 
                file_info
            )
            
            if thumbnail_info.get('has_thumbnail'):
                logger.info(f"✅ Thumbnail stored in: {thumbnail_info.get('stored_in', 'unknown')}")
            else:
                logger.info("⚠️ No thumbnail extracted")
        ...
        # Log success
        file_type = "🎬 Video" if file_info.get('is_video_file') else "📄 File"
        size_str = format_size(file_info.get('file_size', 0))
        thumb_status = "✅ Supabase" if thumbnail_info.get('stored_in') == 'supabase' else \
                      "✅ Base64" if thumbnail_info.get('stored_in') == 'base64' else \
                      "❌ No thumb"
        
        logger.info(f"{file_type} indexed: {file_info.get('title', 'Unknown')[:60]}")
        logger.info(f"   📊 Size: {size_str} | Quality: {file_info.get('quality', 'Unknown')} | {thumb_status}")
...
async def index_files_background_smart():
    """Background indexing with Supabase thumbnails"""
    if User is None or files_col is None or not user_session_ready:
        logger.warning("⚠️ User session not ready for indexing")
        return
    
    logger.info("📁 Starting file indexing with Supabase thumbnails...")
    
    try:
        # Create indexes
        if files_col is not None:
            indexes = [
                ("channel_message_unique", [("channel_id", 1), ("message_id", 1)], True),
                ("file_unique_idx", [("file_unique_id", 1)], False),
                ("title_search_idx", [("normalized_title", "text")], False),
                ("video_idx", [("is_video_file", 1)], False)
            ]
            
            for name, fields, unique in indexes:
                try:
                    await files_col.create_index(fields, unique=unique, name=name)
                except Exception as e:
        ...
        start_id = last_msg['message_id'] if last_msg else 0
        logger.info(f"🔄 Starting from message ID: {start_id}")
        ...
        async for msg in User.get_chat_history(Config.FILE_CHANNEL_ID, limit=100):
            ...
            if msg.document or msg.video:
                ...
                if success:
                    indexed_count += 1
                    if msg.video or (msg.document and 'video' in (msg.document.mime_type or '').lower()):
                        video_count += 1
        ...
        # Statistics
        after_count = await files_col.count_documents({"channel_id": Config.FILE_CHANNEL_ID})
        total_videos = await files_col.count_documents({
            "channel_id": Config.FILE_CHANNEL_ID,
            "is_video_file": True
        })
        ...
        logger.info("=" * 50)
        logger.info(f"✅ New files indexed: {indexed_count}")
        logger.info(f"🎬 Video files: {video_count}")
        logger.info(f"📁 Total in DB: {after_count}")
        logger.info(f"🎬 Total videos in DB: {total_videos}")
        logger.info(f"🔧 Supabase: {'✅ ENABLED' if supabase_storage.enabled else '❌ DISABLED'}")
...
# ============================================================================
# ✅ COMBINED SEARCH WITH SUPABASE THUMBNAILS
# ============================================================================
...
async def search_movies_multi_channel_merged(query, limit=12, page=1):
    ...
    logger.info(f"🔍 Search with Supabase thumbnails: {query}")
    ...
    # ============================================================================
    # ✅ 2. SEARCH FILE CHANNEL (BOT SESSION) - WITH SUPABASE THUMBNAILS
    # ============================================================================
    if files_col is not None:
        try:
            cursor = files_col.find(
                ...
                try:
                    ...
                    if norm_title not in files_dict:
                        ...
                        files_dict[norm_title] = {
                            ...
                            'thumbnail_info': doc.get('thumbnail_info', {})
                        }
                    else:
    ...
    for norm_title in all_titles:
        ...
        if post_data and file_data:
            ...
            result['file_unique_id'] = file_data.get('file_unique_id', '')
            result['has_thumbnail'] = file_data.get('has_thumbnail', False)
            result['thumbnail_info'] = file_data.get('thumbnail_info', {})
    ...
    # Update merged dict with poster data and Supabase thumbnails
    for movie in movies_with_posters:
        ...
        if norm_title in merged:
            merged[norm_title].update({
                ...
                'thumbnail': movie['thumbnail'],  # TMDB poster as thumbnail
                'thumbnail_source': movie['thumbnail_source'],
                'has_poster': movie['has_poster']
            })
            
            # ✅ Add Supabase thumbnail for video files
            if merged[norm_title].get('is_video_file') and merged[norm_title].get('has_thumbnail'):
                supabase_thumbnail = await VideoThumbnailExtractor.get_thumbnail_for_file(merged[norm_title])
                if supabase_thumbnail:
                    merged[norm_title]['video_thumbnail'] = supabase_thumbnail
                    merged[norm_title]['video_thumbnail_source'] = merged[norm_title].get('thumbnail_info', {}).get('stored_in', 'unknown')
                    merged[norm_title]['has_video_thumbnail'] = True
            
            # Thumbnail API endpoint
            if merged[norm_title].get('file_unique_id'):
                merged[norm_title]['thumbnail_api'] = f"{Config.BACKEND_URL}/api/thumbnail/{merged[norm_title]['file_unique_id']}"
    ...
    result_data = {
        ...
        'search_metadata': {
            'query': query,
            'stats': stats,
            'supabase_enabled': supabase_storage.enabled,
...
@app.route('/api/thumbnail/<file_unique_id>', methods=['GET'])
async def api_get_thumbnail(file_unique_id):
    """Get thumbnail URL for a file"""
    try:
        if not file_unique_id:
            return jsonify({'error': 'File ID required'}), 400
        
        # Database से file info प्राप्त करें
        file_info = await files_col.find_one({
            'file_unique_id': file_unique_id
        }, {
            'title': 1,
            'thumbnail_info': 1,
        ...
        thumbnail_url = await VideoThumbnailExtractor.get_thumbnail_for_file(file_info)
        
        if thumbnail_url:
            return jsonify({
                'thumbnail_url': thumbnail_url,
                'source': file_info.get('thumbnail_info', {}).get('thumbnail_source', 'unknown'),
                'stored_in': file_info.get('thumbnail_info', {}).get('stored_in', 'unknown'),
                'has_thumbnail': True,
                'is_video': file_info.get('is_video_file', False),
                'title': file_info.get('title', '')
            })
        else:
            return jsonify({
...
@app.route('/api/thumbnail/direct/<file_unique_id>', methods=['GET'])
async def api_direct_thumbnail(file_unique_id):
    """Direct thumbnail serving (redirect or data)"""
    try:
        # Database से file info प्राप्त करें
        file_info = await files_col.find_one({
            'file_unique_id': file_unique_id
        ...
        if not file_info or not file_info.get('has_thumbnail'):
            # Redirect to fallback
            return redirect(Config.FALLBACK_POSTER)
        
        thumbnail_info = file_info.get('thumbnail_info', {})
        
        # Supabase URL - redirect directly
        if thumbnail_info.get('stored_in') == 'supabase':
            thumbnail_url = thumbnail_info.get('thumbnail_url')
            if thumbnail_url:
                return redirect(thumbnail_url)
        
        # Base64 data - serve directly
        elif thumbnail_info.get('stored_in') == 'base64':
            thumbnail_data = thumbnail_info.get('thumbnail_data', '')
            if thumbnail_data:
        ...
        logger.error(f"Direct thumbnail error: {e}")
        return redirect(Config.FALLBACK_POSTER)
...
async def init_system():
    ...
    try:
        logger.info("🚀 Starting SK4FiLM v8.2 - SUPABASE THUMBNAIL STORAGE...")
        ...
        if bot_session_ready and files_col is not None:
            ...
            logger.info("✅ Started file indexing with Supabase thumbnails")
            await channel_sync_manager.start_sync_monitoring()
            logger.info("✅ Started sync monitoring")
        ...
        logger.info(f"   • Supabase Storage: {'✅ ENABLED' if supabase_storage.enabled else '❌ DISABLED'}")
...
async def root():
    ...
    if files_col is not None:
        ...
        videos_with_thumb = await files_col.count_documents({
            'is_video_file': True,
            'has_thumbnail': True
        })
    else:
    ...
    return jsonify({
        'status': 'healthy',
        'service': 'SK4FiLM v8.2 - SUPABASE THUMBNAIL STORAGE',
        'sessions': {
            'user_session': {
        ...
        'features': {
            ...
            'supabase_storage': supabase_storage.enabled,
        ...
        'statistics': {
            ...
            'videos_with_thumbnails': videos_with_thumb
        },
...
async def api_stats():
    ...
    try:
        ...
        return jsonify({
            ...
            'supabase': {
                'enabled': supabase_storage.enabled,
                'bucket': supabase_storage.supabase_bucket if supabase_storage.enabled else None
            },
            'cache': {
...
if __name__ == "__main__":
    ...
    logger.info("🎯 Features: Supabase Thumbnail Storage Integrated")
