...
async def add_headers(response):
    ...
    response.headers['X-SK4FiLM-Version'] = '8.0-SYNC-FIXED'
...
class PosterFetcher:
    ...
    async def fetch_poster(self, title: str, year: str = ""):
        ...
        logger.info(f"🌐 Fetching poster for: {title} ({year})")
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    # Test file fetch
                    async for msg in User.get_chat_history(Config.FILE_CHANNEL_ID, limit=2):
                        if msg and (msg.document or msg.video):
                            logger.info(f"✅ Can fetch files from FILE channel: YES")
                            break
                    
                    user_session_ready = True
                    
                except Exception as e:
                    logger.error(f"❌ FILE Channel access issue: {e}")
                    logger.error(f"   Make sure you're admin of channel {Config.FILE_CHANNEL_ID}!")
                    user_session_ready = False
                ...
                if "AUTH_KEY_UNREGISTERED" in error_msg:
                    logger.critical("🚨 AUTH KEY INVALID!")
                    break
                    
                if "API_ID_INVALID" in error_msg:
                    logger.critical("🚨 API_ID INVALID!")
                    break
...
async def warm_up_cache():
    ...
    try:
        ...
        # Pre-cache empty movies array for fast response
        empty_movies_cache = {
            'status': 'success',
            'movies': [],
            'total': 0,
        ...
        if cache_manager:
            await cache_manager.set("home:movies:empty", empty_movies_cache, expire_seconds=60)
...
@async_cache_with_ttl(maxsize=1, ttl=60)  # Cache for 1 minute
async def get_home_movies_telegram(limit=30):
    """Fetch 30 real movies directly from Telegram - FAST WITH CACHE"""
    try:
        if not User or not user_session_ready:
            logger.warning("❌ User session not ready")
            return []
        
        movies = []
        seen_titles = set()
        
        logger.info(f"🎬 Fetching {limit} real movies from Telegram...")
        
        # First check cache for fast response
        cache_key = f"home:movies:fast"
        if cache_manager:
            cached_movies = await cache_manager.get(cache_key)
            if cached_movies:
                logger.info("⚡ FAST CACHE HIT for home movies")
        ...
        # Cache results for fast subsequent requests
        if cache_manager and movies:
            await cache_manager.set(cache_key, movies, expire_seconds=300)  # 5 minutes
...
async def search_movies_multi_channel(query, limit=12, page=1):
    ...
    try:
        if files_col is not None:
            # Search in FILE_CHANNEL_ID files
            cursor = files_col.find(
                {
                    "channel_id": Config.FILE_CHANNEL_ID,
                    "$or": [
...
async def search_movies_api(query, limit=12, page=1):
    ...
    try:
        ...
        try:
            ...
            if cache_manager:
                cached = await cache_manager.get(f"search:{query}:{page}:{limit}")
                if cached:
                    return cached
...
async def get_home_movies_live():
    """Optimized home movies with cache"""
    try:
        # First check cache
        cache_key = "home:movies:live"
        if cache_manager:
            cached_movies = await cache_manager.get(cache_key)
            if cached_movies:
                logger.info("⚡ CACHE HIT for home movies")
        ...
        # Cache results
        if cache_manager and movies:
            await cache_manager.set(cache_key, movies, expire_seconds=300)
...
async def init_system():
    ...
    try:
        ...
        if user_session_ready:
            ...
            logger.info("✅ Started SMART background indexing for FILE_CHANNEL_ID")
        else:
        ...
        logger.info(f"   • FILE_CHANNEL_ID: {Config.FILE_CHANNEL_ID}")
        logger.info(f"   • Fast Cache: ENABLED")
        logger.info(f"   • Delete sync: ENABLED")
        logger.info(f"   • Movies Source: Telegram")
