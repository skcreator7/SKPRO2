...
# REDIS CACHE MANAGER - FIXED VERSION
class RedisCache:
    def __init__(self):
        self.client = None
        self.enabled = False
...
# OPTIMIZED MONGODB INIT WITH DUPLICATE KEY HANDLING - FIXED
async def init_mongodb():
    ...
    try:
        ...
        if 'verifications' not in await db.list_collection_names():
            await db.create_collection('verifications')
            
        files_col = db.files
        verification_col = db.verifications
        
        # Create indexes safely - FIXED for duplicate key errors
        ...
        if 'msg_ch_idx' not in existing_indexes:
            try:
                await files_col.create_index(
                    ...
                    name="msg_ch_idx",
                    unique=False  # Changed from unique to non-unique
                )
                logger.info("✅ Created message_channel index (non-unique)")
            except Exception as e:
...
# Database connection check function - ADDED MISSING
    ...
    try:
        if mongo_client:
            ...
            if db and files_col:
                ...
                logger.info(f"✅ Database connection OK, documents: {count}")
                return True
        return False
    except Exception as e:
...
async def setup_bot():
    if bot is None:
        logger.error("❌ Bot instance is None in setup_bot")
        return
...
# THIS WAS THE MISSING PART - ADD THIS
